...
struct communicateNode {
	struct communicate* com;    /**< 通信设备信息 */
	int id;                     /**< 设备ID（0=LoRa, 1=WiFi, 2=蓝牙, 3=LAN） */
	bool enabled;               /**< 使能位，标识是否注册成功 */
	struct communicateNode* next; /**< 指向下一个节点 */
	communicateNode(int id, communicateNode *next) : com(NULL), id(id), enabled(false), next(next) {}
	communicateNode() : com(NULL), id(-1), enabled(false), next(NULL) {}
};
...
private:
	...
	struct communicate* createComnode();

	/**
	 * @brief 创建带fd的通信节点（无超时）
	 * @param fd 设备文件描述符
	 */
	struct communicate* createComnode(int fd);
